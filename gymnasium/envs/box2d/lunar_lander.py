    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        if self.continuous:
            ...
            self.action_space = spaces.Discrete(11) # Engine off, and 10 discrete angles w/engine on
    ...
    def reset(
        ...
        *,
        ...
        for i in [-1, +1]:
            ...
            leg.color1 = (255, 0, 0) # (128, 102, 230)
            leg.color2 = (255, 0, 0) #(77, 77, 128)
    ...
    def map_discrete_state_to_angle(self, action, num_actions=10, min_angle=-10, max_angle=10):
        ...
        angle_step = angle_range / (num_actions - 1)  # -1 to account for the inclusive range
        mapped_angle = min_angle + action * angle_step
    ...
    def step(self, action):
        ...
        if (self.continuous and action[0] > 0.0) or (
            ...
            if self.render_mode is not None:
                ...
                p.ApplyLinearImpulse(
                    (
                        rotated_ox * MAIN_ENGINE_POWER * m_power,
                        rotated_oy * MAIN_ENGINE_POWER * m_power,
                    ),
                    impulse_pos,
                    True,
                )
            self.lander.ApplyLinearImpulse(
                (-rotated_ox * MAIN_ENGINE_POWER * m_power, -rotated_oy * MAIN_ENGINE_POWER * m_power),
                impulse_pos,
                True,
            )
        ...
        if (self.continuous and np.abs(action[1]) > 0.5) or (
            ...
            if self.continuous:
                ...
                s_power = 0.0 # np.clip(np.abs(action[1]), 0.5, 1.0)
